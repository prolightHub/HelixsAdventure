...
var IceDragon = function(xPos, yPos, width, height)
{
    ...
    this.points.update = function(array)
    {
        var array = this;

        if(array[0])
        {
            this.moveAlongTrack(array[0], 0);
        }
        ...
        for(var i = 1; i < array.length; i++)
        {
            ...
            if(last)
            {
                this.moveAlongTrack(array[i], i);
                
                if(Math.pow(array[i].xPos - last.xPos, 2) + Math.pow(array[i].yPos - last.yPos, 2) > self.separation)
                {
                    if(!array[i].oldSpeed) // Double use this variable
                    {
                        array[i].oldSpeed = array[i].speed;
                        array[i].speed = array[i].oldSpeed * 3;
                    }
                }
                else if(array[i].oldSpeed)
                {
                    array[i].speed = array[i].oldSpeed;
                    delete array[i].oldSpeed; // Double use this variable
                }
            }
        } 
    ...
    this.trackNodes.add = function(xPos, yPos)
    {
        this.push({
            xPos : xPos,
            yPos : yPos
    ...
    {
        this.trackNodes.add(Math.round(this.toAddTrackNodes[i].xPos), Math.round(this.toAddTrackNodes[i].yPos));
    }
    ...
    // for(var len = i = this.trackNodes.length - 1; i >= len - 6; i--)
    // {
    //     this.points.add(this.trackNodes[i].xPos, this.trackNodes[i].yPos, 1);
    // }
    ...
    this.dragNodes = function()
    {
        ...
        {
            // Would be our target angle
           
            // var a = this.lastNodes[i + this.points.length - 1].a;

            // if(i > 0)
            // {
            //     // (this.points[i].angle - this.points[i - 1].angle) / 
            // }

            // var a = this.nodes[j - i].a;
            // if(a === undefined)
            // {
            //     a = this.points[i].angle;
            // }

            // a += (this.points[i].angle - this.points[i].lastAngle) / 30;

            // if(this.points[i].angle - a > 0)
            // {
            //     a += 5;
            // }else{
            //     a -= 5;
            ...
            // var lastAngle = (a === this.points[i].angle) ? this.points[i].angle : lastAngle;

            // var a = this.nodes[j - i].a;

            // if(a === undefined)
            // {
            //     a = degrees(this.points[i].angle);
            // }

            // a += (degrees(this.points[i].lastAngle) - degrees(this.points[i].angle)) / 30; 

            var curAngle = radians(physics.formulas.resolveAngle(degrees(this.points[i].angle) + 90));

            // var node = this.nodes[j - i];
            // var newAngle = atan2(this.points[i].yPos - node.yPos, this.points[i].xPos -node.xPos);
        ...
        // var a = radians(this.points[this.points.length - 1].angle);
        // this.nodes.push({
        //     xPos : this.points[this.points.length - 1].xPos - cos(a) * 50, 
        //     yPos : this.points[this.points.length - 1].yPos - sin(a) * 50
        // });
        // var a = radians(this.points[0].angle);
        // this.nodes.push({
        //     xPos : this.points[0].xPos - cos(a) * 50, 
        //     yPos : this.points[0].yPos - sin(a) * 50
        // });
